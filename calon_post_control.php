...
function calon_pc_add_category_restrictions_to_user_panel($user){
	...
	$user_string = get_user_meta($user->ID, 'calon_pc_allowed_categories', TRUE);
	$user_categories = explode(',', $user_string);
	$output = '
		<h3>User Post Restrictions</h3>
		<table class="form-table">
		<tr>
		<th><label for="calon_pc_allowed_categories">Allowed Post Categories</label></th>
		</tr>';
...
add_action('edit_user_profile', 'calon_pc_add_category_restrictions_to_user_panel', 10, 2);

function calon_pc_save_category_restrictions_to_user_panel($user_id){
	...
	if($allowed_categories){
    	update_user_meta($user_id, 'calon_pc_allowed_categories', implode(',', $allowed_categories) );
	}
	else {
		update_user_meta($user_id, 'calon_pc_allowed_categories', '');
	}
}
add_action('profile_update', 'calon_pc_save_category_restrictions_to_user_panel', 10, 2);

function calon_pc_add_email_settings_to_categories_edit_panel($term){
	$t_id = $term->term_id;
 	//echo "<script>console.log('Debug Exclusions: " . $t_id . "' );</script>";
    $send_category_email = get_term_meta($term_id = $t_id, $key = 'calon_pc_send_category_email', TRUE);
    $category_email = get_term_meta($term_id = $t_id, $key = 'calon_pc_category_email', TRUE);
	...
	$output .= '<tr><td><label for="calon_pc_send_category_email">Send Email on Publish</label></td><td><select name="calon_pc_send_category_email" id="calon_pc_send_category_email">
	...
	$output .= '<tr><td><label for="calon_pc_category_email">Send to Email</label></td><td><input type="email" id="calon_pc_category_email" name="calon_pc_category_email" value="' . $category_email . '"></td></tr>';
	$output .='</table><br>';																								 
	echo $output;
}
...
function save_email_settings_to_categories_panel($term_id){
	...
	if (isset( $_POST['calon_pc_category_email'] ) && !is_null($_POST['calon_pc_category_email']) ) {
        $email = sanitize_text_field($_POST['calon_pc_category_email']);
		update_term_meta($term_id = $t_id, $meta_key = 'calon_pc_category_email', $meta_value = $email);
    }
}
add_action('edited_category', 'calon_pc_save_email_settings_to_categories_panel', 10, 2);
add_action('create_category', 'calon_pc_save_email_settings_to_categories_panel', 10, 2);

function calon_pc_force_category_for_users($post_id, $post) {
...
function calon_pc_send_mail_on_post_publish($post_id, $post){
	...
	foreach($categories as $category){
		...
		$send_category_email = get_term_meta($term_id = $category, $key = 'calon_pc_send_category_email', TRUE);
    	$category_email = get_term_meta($term_id = $category, $key = 'calon_pc_category_email', TRUE);
		if($send_category_email){
...
add_action( 'publish_post', 'calon_pc_send_mail_on_post_publish', 10, 3 );
